...
# modified from from https://github.com/igul222/improved_wgan_training
def Layernorm(input):

    with tf.variable_scope("layernorm"):
        norm_axes=[1,2,3]
        channels = input.get_shape()[3]
        mean, var = tf.nn.moments(input, axes=norm_axes, keep_dims=True)

        # Assume the 'neurons' axis is the first of norm_axes. This is the case for fully-connected and BCHW conv layers.
        n_neurons = input.get_shape().as_list()[norm_axes[0]]
        ...
        scale = tf.get_variable("scale", n_neurons, dtype=tf.float32,
                                ...
                                initializer=tf.random_normal_initializer(1.5, 0.02)
                                )
...
def load_examples(a):
    ...
    with tf.name_scope("load_images"):
        ...
        if a.lab_colorization:
            ...
            #im_width = width // 3
            #temp_images = preprocess(raw_input[:, :im_width, :])
            #m_images = preprocess(raw_input[:, im_width:im_width*2, :])
            #b_images = preprocess(raw_input[:, im_width*2:, :])
            #a_images = tf.abs(tf.div(temp_images, m_images))
...
def create_generator_Unet(a, generator_inputs, generator_outputs_channels):
    layers = []
...
def save_images(a, fetches, step=None):
    ...
    for i, in_path in enumerate(fetches["paths"]):
        #print('fetches paths', fetches["paths"])
...
def gen_loss_sgan(D_fake,a):
    """

    :return:
    """
    if a.f_type == 'variational':
        D_fake = 0.5 * tf.tanh(D_fake)
        G_loss = -tf.reduce_mean(tf.exp(-D_fake))
    elif a.f_type == 'pearson':
        D_fake = -D_fake
        G_loss = -tf.reduce_mean(-0.75 * D_fake ** 2 + 4* D_fake-8.0/3.0)
    elif a.f_type == 'helinger':
        D_fake = 1 - tf.exp(D_fake)
        G_loss = -tf.reduce_mean(tf.exp(D_fake)-1)
    elif a.f_type=='cap':
        D_fake =  -tf.log(1.0+tf.exp(-D_fake))
        G_loss = -tf.reduce_mean(tf.log(1+tf.exp(-D_fake)))
    elif a.f_type=='symkl':
        D_fake =  -tf.log(1.0+tf.exp(-D_fake))
        G_loss = -tf.reduce_mean(tf.exp(-D_fake)-D_fake-1)
    return G_loss

def discrim_loss_sgan(D_real, D_fake,a):
    """
    :return:
    """
    if a.f_type=='variational':
        D_real = 0.5*tf.tanh(D_real)
        D_fake = 0.5*tf.tanh(D_fake)

        def phi(alpha):
            return tf.exp(-alpha)

        D_loss = (tf.reduce_mean(phi(-D_fake)) + tf.reduce_mean(phi(D_real)))
    elif a.f_type=='pearson':
        D_fake = -D_fake
	D_real = -D_real
        def phi(alpha):
            return -0.75 * alpha ** 2 + 4* alpha-8.0/3.0
        D_loss = (tf.reduce_mean(phi(-D_fake))+tf.reduce_mean(phi(D_real)))
    elif a.f_type=='helinger':
        D_real = 1-tf.exp(-D_real)
        D_fake = 1-tf.exp(-D_fake)
        def phi(alpha):
            return tf.exp(alpha)-1
        D_loss = (tf.reduce_mean(phi(-D_fake))+tf.reduce_mean(phi(D_real)))
    elif a.f_type=='cap':	
        D_fake = -tf.log(1.0+tf.exp(-D_fake))
        D_real = -tf.log(1.0+tf.exp(-D_real))
        def phi(alpha):
            return tf.log(1+tf.exp(alpha))
        D_loss = (tf.reduce_mean(phi(-D_fake))+tf.reduce_mean(phi(D_real)))
    elif a.f_type=='symkl':
        D_fake =  -tf.log(1.0+tf.exp(-D_fake))
        D_real = -tf.log(1.0+tf.exp(-D_real))
        def phi(alpha):
            return tf.exp(-alpha)-alpha-1
        D_loss = (tf.reduce_mean(phi(-D_fake))+tf.reduce_mean(phi(D_real)))
    return -D_loss 
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
